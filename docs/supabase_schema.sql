...
create index if not exists idx_ai_posture_analyses_user_time
...
drop policy if exists "ai_posture_analyses_own_all" on public.ai_posture_analyses;
...
create policy "ai_posture_analyses_own_all"
on public.ai_posture_analyses for all
using (auth.uid() = user_id)
with check (auth.uid() = user_id);
